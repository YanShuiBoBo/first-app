...
【3. 知识卡片 knowledge】
6. 目标密度：请确保知识点覆盖全面，粒度更细、宁多勿少（但要准确、贴合语境）。
   - 不仅提取生僻难词，也要提取：
     - 简单词的地道用法（如 "do" 的特殊含义）；
   ...
   - 数量建议（根据输入字幕长度自适应）：
     * 尽量做到“覆盖所有值得学的词/短语/表达”，不要只挑最显眼的少数几个。
   - 粒度要求：
     * 同一句里同时出现「单词 + 固定搭配 + 常用表达」时，请拆成多张卡片分别提取；
     * 触发词优先使用学习者会在字幕里“看到并想点”的形式（不要把整句当 trigger_word）。
